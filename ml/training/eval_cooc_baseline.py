...
def main():
    ...
    ap.add_argument("--items", required=False, default="")
    ...
    if pur.empty:
        raise SystemExit("No purchases available for baseline eval")
    ...
    price_map = items_lookup["price"].to_dict() if ("price" in items_lookup.columns and not items_lookup.empty) else {}
    ...
    for r in ds.itertuples(index=False):
        ...
        for pid in top10:
            p = _to_float(price_map.get(int(pid)))
            if p is None:
                continue
            rec_price_sum10 += p
            rec_price_cnt10 += 1
    ...
    catalog_size = int(len(items_lookup.index)) if not items_lookup.empty else int(pur["item_id"].nunique())
