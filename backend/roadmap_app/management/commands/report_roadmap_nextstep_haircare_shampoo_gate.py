...
from roadmap_app.nextstep_haircare_shampoo_gate import (
    ...
    render_nextstep_haircare_shampoo_gate_markdown,
)


class Command(BaseCommand):
    help = "Build a narrow read-only shampoo-gate report for roadmap nextstep historical anchors."
    ...
    def handle(self, *args, **options):
        ...
        markdown = render_nextstep_haircare_shampoo_gate_markdown(payload)

        fmt = str(options.get("format") or "both")
        if fmt in {"json", "both"}:
            json_path = out_stem.with_suffix(".json")
            json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            self.stdout.write(f"Shampoo gate JSON written to `{json_path}`")
        if fmt in {"md", "both"}:
            md_path = out_stem.with_suffix(".md")
            md_path.write_text(markdown, encoding="utf-8")
            self.stdout.write(f"Shampoo gate markdown written to `{md_path}`")
