...
class TransactionsCheckoutContractTests(APITestCase):
    def setUp(self):
        ...
        silver, _ = Tier.objects.get_or_create(
            name="Silver",
            defaults={"threshold_spend_90d": Decimal("100.00"), "points_rate": Decimal("0.10")},
        )
        silver.threshold_spend_90d = Decimal("100.00")
        silver.points_rate = Decimal("0.10")
    ...
    def test_transactions_and_last_checkout_expose_rich_snapshot(self):
        ...
        self.assertEqual(checkout.data["discount_amount"], "0.00")
        self.assertEqual(checkout.data["net_total"], "120.00")
        self.assertEqual(checkout.data["points_earned"], 12)
        self.assertEqual(checkout.data["points_redeemed"], 0)
        self.assertEqual(checkout.data["new_balance"], 12)
        ...
        self.assertEqual(txn["discount_amount"], "0.00")
        self.assertEqual(txn["net_total"], "120.00")
        self.assertEqual(txn["points_earned"], 12)
        self.assertEqual(txn["points_redeemed"], 0)
        self.assertEqual(txn["points_change"], 12)
        self.assertEqual(txn["new_balance"], 12)
        ...
        self.assertEqual(detail.data["points_change"], 12)
    ...
    def test_checkout_preview_and_commit_redeem_points_reduce_kzt_total(self):
        ...
        self.assertEqual(preview.data["net_total"], "100.00")
        self.assertEqual(preview.data["points_redeemed"], 20)
        self.assertEqual(preview.data["estimated_points_earned"], 10)
        self.assertEqual(preview.data["balance_before"], 50)
        self.assertEqual(preview.data["balance_after_estimated"], 40)

        checkout = self.client.post("/api/checkout", payload, format="json")
        self.assertEqual(checkout.status_code, 201)
        self.assertTrue(checkout.data["ok"])
        self.assertEqual(checkout.data["gross_total"], "120.00")
        self.assertEqual(checkout.data["net_total"], "100.00")
        self.assertEqual(checkout.data["points_redeemed"], 20)
        self.assertEqual(checkout.data["points_earned"], 10)
        self.assertEqual(checkout.data["new_balance"], 40)
